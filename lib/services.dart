...
class _ServicesState extends State<Services> {
  ...
  double get averageServicePrice {
    ...
    return services.fold(
          0.0,
          (sum, s) => sum + (s.discountedPrice ?? s.price ?? 0),
        ) /
        services.length;
  }
  ...
  Future<void> _exportToCopy() async {
    try {
      ...
      buffer.writeln(
        'Name\tCategory\tPrice\tDisc. Price\tDuration\tGender\tStatus\tOnline Booking',
      );
      for (var s in filteredServices) {
        buffer.writeln(
          '${s.name ?? ''}\t${s.category ?? ''}\t₹${s.price ?? 0}\t₹${s.discountedPrice ?? s.price ?? 0}\t${s.duration ?? 0} min\t${s.gender ?? 'unisex'}\t${s.status ?? ''}\t${s.onlineBooking == true ? 'Yes' : 'No'}',
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '${filteredServices.length} services copied to clipboard!',
    ...
    try {
      ...
      for (var s in filteredServices) {
        rows.add([
          ...
          s.category ?? '',
          s.price ?? 0,
          s.discountedPrice ?? s.price ?? 0,
          s.duration ?? 0,
          s.gender ?? 'unisex',
          s.status ?? '',
          s.onlineBooking == true ? 'Yes' : 'No',
        ]);
      }
      String csv = const ListToCsvConverter().convert(rows);
      final directory = await getApplicationDocumentsDirectory();
      final path =
          '${directory.path}/services_export_${DateTime.now().millisecondsSinceEpoch}.csv';
    ...
    try {
      ...
      for (int i = 0; i < filteredServices.length; i++) {
        ...
        sheetObject
            ...
            .value = TextCellValue(
          (s.discountedPrice ?? s.price ?? 0).toString(),
        );
        sheetObject
            .cell(
      ...
      var filePath =
          '${directory.path}/services_export_${DateTime.now().millisecondsSinceEpoch}.xlsx';
  ...
  Future<void> _exportToPDF() async {
    try {
        ...
        pw.MultiPage(
          ...
          build: (context) => [
            pw.Header(
              level: 0,
              child: pw.Text(
                'Services Report - ${DateFormat('yyyy-MM-dd').format(DateTime.now())}',
              ),
            ),
      ...
      final directory = await getApplicationDocumentsDirectory();
      final path =
          '${directory.path}/services_export_${DateTime.now().millisecondsSinceEpoch}.pdf';
      final file = File(path);
                ...
                Transform.scale(
                  ...
                  child: Switch(
                    ...
                    onChanged: (val) async {
                      ...
                      final originalIndex = services.indexOf(
