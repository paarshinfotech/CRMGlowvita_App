...
class _AllProductsReportState extends State<AllProductsReport> {
  ...
  Future<void> _fetchReport() async {
    ...
    try {
      final result = await ApiService.getProductSummaryReport();
    ...
    return _wrapperTable(
      DataTable(
        ...
        columns: const [
          ...
          DataColumn(label: Text('Product Form')),
        ...
        rows: displayedData.map((p) {
          final dateStr = p['createdAt'] != null
              ? DateFormat('M/d/yyyy').format(DateTime.parse(p['createdAt']))
              : '—';
          final pName = p['productName'] ?? '—';
          final brand = p['brand'] ?? '—';
          final cat = p['category'] ?? '—';
          final pForm = p['productForm'] ?? '—';
          final price = p['price'] != null ? _fmt(p['price']) : '—';
          final salePrice = p['salePrice'] != null ? _fmt(p['salePrice']) : '—';
          final stock = p['stock']?.toString() ?? '0';
