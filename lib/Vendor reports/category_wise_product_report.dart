...
class _CategoryWiseProductReportState extends State<CategoryWiseProductReport> {
  ...
  Widget build(BuildContext context) {
    int totalProducts = _filtered.fold(0, (sum, c) => sum + ((c['numberOfProducts'] as num?)?.toInt() ?? 0));
    int activeProducts = _filtered.fold(0, (sum, c) => sum + ((c['activeProducts'] as num?)?.toInt() ?? 0));
